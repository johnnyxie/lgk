...
export default class GridStack extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        let children = React.Children.toArray(this.props.children);
        let pChildren = React.Children.toArray(prevProps.children);
        ...
        const toRemove = differenceWith(pChildren, children, (a, b) => {
            return (a.props.id === b.props.id)
        })

        // Find new widgets or those we need to re-render
        const uniques = children.filter(newChild => {
            for (const index in pChildren) {
                const oldChild = pChildren[index]
    ...
    render() {
        return (
            <div ref="gridstack" className="grid-stack" style={{marginTop: 10}} />
        )
    }
}
